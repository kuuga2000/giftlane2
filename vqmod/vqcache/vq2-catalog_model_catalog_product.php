...
class ModelCatalogProduct extends Model {
	...
	public function getProducts($data = array()) {
		...
		$sql .= " LEFT JOIN oc_product_to_category pc2 ON p.product_id=pc2.product_id 
LEFT JOIN oc_category sc ON sc.category_id=pc2.category_id LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p.status = '1' AND sc.status='1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'";
	...
	/**
	* new function to get just in products
	* @input: array of filter like category @id
	* @return: just in products
	*/
	public function getJustinProducts($data = array(), $intRand = 0) {
		if ($this->customer->isLogged()) {
			$customer_group_id = $this->customer->getCustomerGroupId();
		} else {
			$customer_group_id = $this->config->get('config_customer_group_id');
		...
		$sql .= " AND justin = 1 AND display='yes' ";
		
		if(!empty($data['oldProducts']))
		{
			$str = '';
			$oldProductsArr = explode(',',$data['oldProducts']);
			...
			$str = trim($str,',');
			$sql .= " AND p.product_id NOT IN(".$str.")";
		}
		if (!empty($data['filter_category_id'])) {
			if (!empty($data['filter_sub_category'])) {
				$sql .= " AND cp.path_id = '" . (int)$data['filter_category_id'] . "'";	
	...
	public function getMainCategory($product_id) {
		$strCatQ = "SELECT category_id FROM " . DB_PREFIX . "product_to_category WHERE  product_id = '" . (int)$product_id . "'";
		$query = $this->db->query($strCatQ);
	...
	public function getParentCategories($catId) {
		$query = $this->db->query("SELECT parent_id FROM " . DB_PREFIX . "category WHERE category_id = '" . (int)$catId . "' AND parent_id!=0 LIMIT 0,1");
...
LEFT JOIN oc_category sc ON sc.category_id=pc2.category_id LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p.status = '1' AND sc.status='1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'";
	...
	/**
	* new function to get count of just in products
	* @input: array of filter like category @id
	* @return: just in products
	*/
	public function getTotlaJustInProducts($data = array()) {
		if ($this->customer->isLogged()) {
			$customer_group_id = $this->customer->getCustomerGroupId();
		} else {
			$customer_group_id = $this->config->get('config_customer_group_id');
	...
	public function getTotalUpsellProducts($data = array()) {
		if ($this->customer->isLogged()) {
			$customer_group_id = $this->customer->getCustomerGroupId();
		} else {
			$customer_group_id = $this->config->get('config_customer_group_id');
	...
	public function getProductOptionQunatity($size,$color,$product_id)
	{
		$sql = "SELECT quantity FROM " . DB_PREFIX . "product_options_new WHERE size = '".$this->db->escape($size)."' AND  color = '".$this->db->escape($color)."' AND product_id = ".(int)$product_id." ";
	//	echo $sql;
		$query = $this->db->query($sql);
		if($query->num_rows > 0)
		{
	...
	{
		$sql = "SELECT DISTINCT(color) FROM " . DB_PREFIX . "product_options_new WHERE product_id = ".(int)$productId." AND color !='' ORDER by color ";
		$query = $this->db->query($sql);
		return $query->rows;
	}
	...
	{
		$sql = "SELECT DISTINCT(size) FROM " . DB_PREFIX . "product_options_new WHERE product_id = ".(int)$productId." AND size != '' ";
		$query = $this->db->query($sql);
		return $query->rows;
	}
	
	public function getProductColorSize($option, $product_id)
	{
		$sql = "SELECT DISTINCT(size) FROM " . DB_PREFIX . "product_options_new WHERE product_id = ".(int)$product_id." AND color = '".$this->db->escape($option)."'  AND size != '' ";
	
		$query = $this->db->query($sql);
		return $query->rows;
	}
	
	public function getProductColorQunatity($option, $product_id){
		$sql = "SELECT quantity AS quantity FROM " . DB_PREFIX . "product_options_new WHERE product_id = ".(int)$product_id." AND color = '".$this->db->escape($option)."' AND size = '' ";
		$query = $this->db->query($sql);
		return $query->row['quantity'];
	}
	...
	public function isUpsellProduct($productId)
	 {
	  ...
	  $query = $this->db->query("SELECT category_id  FROM `" . DB_PREFIX . "category_description` WHERE `name` LIKE 'Standard Greeting card%' OR name LIKE 'Premium Greeting card%' OR name LIKE 'Standard Wrapping Paper%' OR name LIKE 'Premium Wrapping Paper%'  ");
	   $categories = '';
	  foreach ($query->rows as $result) { 
	...
	//to get all products just in and all would be retrieved all
	public function getAllProducts($data = array(), $intRand = 0) {
		if ($this->customer->isLogged()) {
			$customer_group_id = $this->customer->getCustomerGroupId();
		} else {
			$customer_group_id = $this->config->get('config_customer_group_id');
		...
		$sql .= " AND display='yes' /*justin = 1*/ /*OR justin=0*/ /*AND pd.name NOT LIKE 'Wra%'*/";
		...
		if($intRand == 1)
		{
				$sql .= " ORDER BY product_id /*ORDER BY rand()*/";	
		}
		else
		{
			if (isset($data['sort']) && in_array($data['sort'], $sort_data)) {
	...
	//to get total products just in and all would be retrieved all
	public function getTotlaOldProducts($data = array()) {
		if ($this->customer->isLogged()) {
			$customer_group_id = $this->customer->getCustomerGroupId();
		} else {
			$customer_group_id = $this->config->get('config_customer_group_id');
		...
		$sql .= " AND display = 'yes' /*OR justin=0*/";
		if (!empty($data['filter_category_id'])) {
			if (!empty($data['filter_sub_category'])) {
				$sql .= " AND cp.path_id = '" . (int)$data['filter_category_id'] . "'";	
			} else {
