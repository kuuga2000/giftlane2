...
class ControllerSaleContact extends Controller {
	...
	public function listemails(){
		...
		$dataProvider = $this->db->query("SELECT * FROM oc_newsletter ORDER BY email ASC LIMIT $start,$stop")->rows;
		$countData = $this->db->query("SELECT * FROM oc_newsletter")->num_rows;
		//echo $countData;exit;
		
		
		$this->data['dataProvider'] = $dataProvider;
		$this->data['sumData'] = $countData;
		$this->data['page'] = $page;
		$this->data['sumPage'] = $sumPage = ceil(($countData/$stop));
		$this->data['href'] = $this->url->link('sale/contact/listemails', 'token=' . $this->session->data['token'], 'SSL');
		
		$this->template = 'sale/listemails.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
