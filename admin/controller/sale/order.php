...
class ControllerSaleOrder extends Controller {
	private $error = array();
  	...
  	public function update() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			//echo $this->request->post['order_status_id']15;exit;
			//echo "submit edit order";exit;
			$this->model_sale_order->editOrder($this->request->get['order_id'], $this->request->post);
	  		
			
			$order_Details = $this->model_sale_order->getOrderStatus($this->request->post['order_status_id']);
			
			//if order status prosecced
			if(isset($this->request->post['orderChanged']) && $this->request->post['orderChanged'] == 1 && strtolower($order_Details['name']) == strtolower('Processed'))
			{
				...
				$customer = $res->row;
				$subject = 'Giftlane - Invoice for your order number '.$this->request->get['order_id'];
				$html = "Dear ".$customer['firstname'];
				$html .= '<br><br>';
				$html .= "Your payment has been processed.";
	
				$html .= "No resi JNE anda ".$this->request->post['no_resi'];
				$html .= "<br><br>Thank You<br><br>Warmest Regards,<br><br>Giftlane Co";
			...
			if(isset($this->request->post['orderChanged']) && $this->request->post['orderChanged'] == 1 && ($order_Details['name'] == 'Processing' || $order_Details['name'] == 'processing'))
			{
				...
				$html = "Dear ".$customer['firstname'];
				$html .= '<br><br>';
				$html .= "Your payment has been approved.";
			...
			if(isset($this->request->post['orderChanged']) && $this->request->post['orderChanged'] == 1 && ($order_Details['name'] == 'Canceled' || $order_Details['name'] == 'canceled'))
			{
				...
				$subject = 'Giftlane -  YOUR ORDER HAS BEEN CANCELLED ';
				$html = "Dear ".$customer['firstname'];
				$html .= '<br><br>';
				$html .= "We are to inform you that your order: ".$this->request->get['order_id']." has been cancelled.<br><br>";
				$html .= "<a href='".HTTP_CATALOG."'>Click this link</a> to browse for more selections.";
				$html .= "<br><br>Thank you for choosing Giftlace Co<br><br>Warmest Regards,<br><br>Giftlane Co";
  	...
  	public function getForm() {
		...
		//no resi
		!empty($order_info['no_resi'])?$this->data['no_resi'] = $order_info['no_resi']:$order_info['no_resi']='';
		//
		...
		*/
		...
		foreach ($order_products as $order_product) {
			...
			$this->data['order_products'][] = array(
				...
				'greetingId'       => $order_product['greetingId'],
				'greetingName'     => $order_product['greetingName'],
				'greetingPrice'    => $order_product['greetingPrice'],
				'wrappingId'       => $order_product['wrappingId'],
				'wrappingName'     => $order_product['wrappingName'],
				'wrappingPrice'    => $order_product['wrappingPrice'],
	...
	public function info() {
		...
		if ($order_info) {
			...
			$this->data['no_resi'] = $order_info['no_resi'];
		...
		$this->data['payment_total'] = $order_info['total'];
			...
			foreach ($products as $product) {
				...
				$this->data['products'][] = array(
					...
					'greetingId'       => $product['greetingId'],
					'greetingName'     => $product['greetingName'],
					'greetingPrice'    => $product['greetingPrice'],
					'wrappingId'       => $product['wrappingId'],
					'wrappingName'     => $product['wrappingName'],
					'wrappingPrice'    => $product['wrappingPrice'],
					'color'            => $product['color'],
					'size'            =>  $product['size']
				);
			}
