...
class ModelLocalisationCity extends Model {
	...
	public function addCity($data) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "city SET status = '" . (int)$data['status'] . "', name = '" . $this->db->escape($data['name']) . "', code = '" . $this->db->escape($data['code']) . "', price = '" . $this->db->escape($data['price']) . "', zone_id = '" . (int)$data['zone_id'] . "'");
			
		$this->cache->delete('city');
	}

	public function editCity($city_id, $data) {
		$this->db->query("UPDATE " . DB_PREFIX . "city SET status = '" . (int)$data['status'] . "', name = '" . $this->db->escape($data['name']) . "', code = '" . $this->db->escape($data['code']) . "', zone_id = '" . (int)$data['zone_id'] . "', price = '" . (int)$data['price'] . "' WHERE city_id = '" . (int)$city_id . "'");
	...
	public function getPrice($city_id) {
		$query = $this->db->query("SELECT price FROM " . DB_PREFIX . "city WHERE city_id = '" . (int)$city_id . "'");
